...
func (ip *Inode) Read(txn *twophase.TwoPhase, offset uint64, bytesToRead uint64) ([]byte, bool) {
	if offset >= ip.Size {
		return nil, true
	}
	...
	buf := txn.ReadBuf(block2addr(ip.Data), common.NBITBLOCK)
	countCopy := count
...
func (ip *Inode) WriteInode(txn *twophase.TwoPhase) {
	d := ip.Encode()
	txn.OverWrite(inum2Addr(ip.Inum), common.INODESZ*8, d)
	util.DPrintf(1, "WriteInode %v\n", ip)
}

// Returns number of bytes written and error
func (ip *Inode) Write(txn *twophase.TwoPhase, offset uint64, count uint64, dataBuf []byte) (uint64, bool) {
	...
	buffer := txn.ReadBuf(block2addr(ip.Data), common.NBITBLOCK)
...
func ReadInode(txn *twophase.TwoPhase, inum common.Inum) *Inode {
	buffer := txn.ReadBuf(inum2Addr(inum), common.INODESZ*8)
...
func inodeInit(txn *twophase.TwoPhase) {
